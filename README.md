...
2) Propagation via Acoustic Analogy [3, 4]:
    ...
    The rotor is assumed positioned at (0, 0, 0) in 3D, with the axis positioned along (0, 0, 1). The zero-azimuth datum is (1, 0, 0). 
    Throughout, the variables in the frequency domain are defined with the Fourier transform convention: \hat{f}_k = \Omega / 2 / \pi \int_0^{2 \pi / \Omega} (f(t) * exp(1j * k * \Omega)) dt. Mind the sign of the exp term!
...
outputs:
    ...
    p_loading_blade_steady_Pa/real, imag : arrays of float of shape (Nx, Nm) - far-field pressure field of the propeller steady loading noise in units Pa at locations defined by observer_position_m and harmonics m of Omega*B defined by harmonics_m. Axis 0 corresponds to observer positions, axis 1 to harmonic number.
    ...
    observer_position_m: array of floats of shape (3, Nx) - observer positions in the far field, in meters. Axis 0 corresponds to the 3 vector components x, y, z. Propeller is positioned at (0, 0, 0) with its axis along (0, 0, 1). The zero azimuth station is taken in the direction (1, 0, 0), corresponding to the strut azimuth.
    ...
    harmonics_m : array of int of shape Nm - integers m defining the harmonics of the blade-passing frequency Omega*B corresponding to the far-field pressure harmonics p_loading_blade_steady_Pa, etc. The corresponding frequency in Hz is: m * B * Omega / 2 /pi.
...
[1]  Vella, E., Gojon, R., Parisot-Dupuis, H., Doué, N., Jardin, T., and Roger, M., “Mutual Interaction Noise in Rotor–strut Configuration,” AIAA Journal, Vol. 0, No. 0, 0, pp. 1–16.
